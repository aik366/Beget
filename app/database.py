...
async def add_column():
    async with aiosqlite.connect('../DATA/user.db') as db:
        await db.execute("ALTER TABLE users ADD data_time TEXT")
...
async def add_db(user_text: str):
    ...
    split_data_list = user_split[2].replace(',', '.').split('.')
    split_data = f"{split_data_list[2]}-{split_data_list[1].zfill(2)}-{split_data_list[0].zfill(2)}"
    split_data = date.fromisoformat(split_data)
    async with aiosqlite.connect('DATA/user.db') as db:
        await db.execute("INSERT INTO users_data (user_surname, user_name, user_data) VALUES (?, ?, ?)",
                         (user_split[0], user_split[1], split_data))
        await db.commit()
...
async def db_select():
    async with aiosqlite.connect('DATA/user.db') as db:
        cursor = await db.execute("SELECT * FROM users_data ORDER BY strftime('%d.%m', user_data) ASC")
...
async def birthday_reminder():
    async with aiosqlite.connect('DATA/user.db') as db:
        cursor = await db.execute("SELECT * FROM users_data WHERE strftime('%d-%m', user_data) = ?", (f'{delta_days}',))
        users = await cursor.fetchall()
        if users:
